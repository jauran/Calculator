Public Class Form1
        ...
        While InStr(Mid(TextBox1.Text, 2), "*") > 0 or InStr(Mid(TextBox1.Text, 2), "/") > 0 '避开首位-号
            For z = 1 To 100
                If InStr(Mid(TextBox1.Text, 2, z), "*") > 0 or InStr(Mid(TextBox1.Text, 2, z), "/") > 0 Then
                    Zuo = Mid(TextBox1.Text, 1, z)
                    Exit For
                End If
            Next
           For zs = 0 To 100  'Zuo1位-zs0位，长度才不会归0报错
                If InStr(Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs), "*") = 0 And InStr(Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs), "/") = 0 And InStr(Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs), "+") = 0 And InStr(Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs), "-") = 0 Then
                     ZuoShu = Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs)
                    Exit For
                End If
            Next

            You = Mid(TextBox1.Text, Len(Zuo) + 2)
            For y = 1 To 100
    ...
    Private Sub PictureBox1_Click(sender As Object, e As EventArgs) Handles PictureBox1.Click
        MsgBox("核心代码：" & vbCrLf &
               "While InStr(Mid(TextBox1.Text, 2), *) > 0 or /> 0" & vbCrLf &
               "   For z = 1 To 100" & vbCrLf &
               "      If InStr(Mid(TextBox1.Text, 2, z), *) > 0 or (..., /) > 0 Then" & vbCrLf &
               "         Zuo = Mid(TextBox1.Text, 1, z)" & vbCrLf &
               "      Exit For   End If   Next" & vbCrLf &
               "   For zs = 0 To 100" & vbCrLf &
               "      If InStr(Microsoft.VB.Right(Zuo,  Len(Zuo) - zs), *) = 0 And /+-= 0 Then" & vbCrLf &
               "         ZuoShu = Microsoft.VisualBasic.Right(Zuo,  Len(Zuo) - zs)" & vbCrLf &
               "      Exit For   End If   Next" & vbCrLf &
               "   You = Mid(TextBox1.Text, Len(Zuo) + 2)" & vbCrLf &
               "   For y = 1 To 100" & vbCrLf &
               "      If InStr(Mid(You, 1, y), *) = 0 And  /+-= 0 Then" & vbCrLf &
               "         YouShu = Mid(You, 1, y)" & vbCrLf &
               "         Else   Exit For" & vbCrLf &
               "      End If   Next" & vbCrLf &
               "   If Mid(TextBox1.Text, Len(ZuoShu) + 1, 1) = * Then" & vbCrLf &
               "   TextBox1.Text =Mid(TextBox1.Text, 1,Len(Zuo) - Len(ZuoShu))" & vbCrLf &
               "                              &  Val(ZuoShu) * Val(YouShu) &" & vbCrLf &
               "             Microsoft.VisualBasic.Right(TextBox1.Text, Len(You) - Len(YouShu))" & vbCrLf &
               "   ElseIf ...= / Then ...& Val(ZuoShu) / Val(YouShu) &..." & vbCrLf &
               "End While" & vbCrLf & "赵然https://github.com/jauran" & vbCrLf & "2020年04月15日使用记事本编写MSBuild编译" & vbCrLf & "其他作品：" & vbCrLf & "文学集《雪华》ISBN978-7-5108-2812-6",, "四则混合运算计算器")
